...
class Day9 {
  ...
  def solve(input: Seq[String]): (Long, Long) = {
    ...
    val yMax = input.size
    val matrix = input.map(_.split("").toSeq.zipWithIndex)
      .zipWithIndex
      .flatMap { case (row, yIdx) => row.map { case (value, xIdx) => Point(xIdx, yIdx, value, xIdx + yIdx * xMax) }
      }

    def part1Old(part1Input: Seq[String]) = {
      def getSliding(inputSequence: Seq[String]): Seq[Seq[(Char, String)]] = inputSequence.map(_.sliding(3).toSeq)
        .map { seq =>
      ...
      (horizontal zip vertical).map { case (h, v) => h._1 -> (h._2 + v._2)
      }.filter { case (value, neighbors) => neighbors.forall(_.toInt > value.toInt) }.map(lowPoint => {
        Integer.parseInt(lowPoint._1.toString) + 1
      }).sum
    }
    ...
    @tailrec
    def expandBasin(startPoint: Point,
                    closed    : Seq[Point] = Seq.empty,
                    open      : Seq[Point] = Seq.empty): Seq[Point] = startPoint match {
      case Point(_, _, "9", _) if open.isEmpty  => closed
      case _ if open.isEmpty && closed.nonEmpty => closed
      case Point(_, _, "9", _)                  => expandBasin(
        open.head, closed :+ open.head, open.drop(1))
      case point                                => pointNeighbors(point).filterNot(closed.contains(_)) match {
        case Seq(neighborHead, neighborTail @ _*) => expandBasin(
          neighborHead, closed :+ point, open.concat(neighborTail))
