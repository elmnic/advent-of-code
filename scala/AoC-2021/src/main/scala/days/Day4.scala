...
class Day4 {
  ...
  case class Board(numbers: Seq[MarkableNumber], bingoNumber: Int = 0) {
    ...
    def printBoard(): Unit = numbers.map(_.number).grouped(5).foreach(row => println(row.mkString(" "))).tap(_ => println())

    def printMarked(): Unit = numbers.map(_.marked).grouped(5).foreach(row => println(row.mkString(" "))).tap(_ => println())
  }
    ...
    val boards: Seq[Board] = input
      ...
      .map { groupOfRows =>
        Board(groupOfRows.flatMap { rowAsString =>
          rowAsString.split(""" +""")
            .filterNot(_.isEmpty)
            .map(number => MarkableNumber(number.toInt, marked = false))
        })
      }.toSeq
