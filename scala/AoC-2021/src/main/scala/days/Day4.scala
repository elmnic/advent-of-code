...
class Day4 {
  ...
  case class Board(numbers: Seq[MarkableNumber], bingoNumber: Int = 0) {
    ...
    def printBoard(): Unit =
      numbers.map(_.number).grouped(5).foreach(row => println(row.mkString(" "))).tap(_ => println())

    def printMarked(): Unit =
      numbers.map(_.marked).grouped(5).foreach(row => println(row.mkString(" "))).tap(_ => println())
  }

  def solve(input: Seq[String]): (Long, Long) = {
    def markNumbers(board: Board, numberToMark: Int): Board =
      Board(board.numbers.map { case MarkableNumber(number, mark) =>
    ...
    val boards: Seq[Board] = input.tail
      .filter(_.nonEmpty)
      .grouped(5)
      .map { groupOfRows =>
        Board(groupOfRows.flatMap { rowAsString =>
          rowAsString.split(""" +""").filterNot(_.isEmpty).map(number => MarkableNumber(number.toInt, marked = false))
        })
      }
      .toSeq
    ...
    val winningBoard = markedBoards.minBy(board => numbersToDraw.indexOf(board.bingoNumber))
    val resultPart1 = winningBoard.bingoNumber * winningBoard.unmarkedSum

    val losingBoard = markedBoards.maxBy(board => numbersToDraw.indexOf(board.bingoNumber))
