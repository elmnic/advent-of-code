  ...
  def solve(input: Seq[String]): (Long, Long) = {
    ...
    def deduce(patterns: Seq[String]): Map[Int, String] = {
      ...
      def deduceTwoFiveSix(patterns: Seq[String], knownNumbers: Map[Int, String]): Map[Int, String] = {
        ...
        val uniqueTo = remainingNumbers.flatten
          .groupBy(identity)
        ...
        knownTwo ++ remainingNumbers
          .filterNot(_ == knownTwo(2))
          .map { pattern =>
            pattern.length match {
              case 5 => 5 -> pattern
              case 6 => 6 -> pattern
      ...
      val withZeroThreeNine = deduceThreeAndNine(getWithOne(patterns, uniqueNumbers), withZero) ++ withZero
      val allKnown = deduceTwoFiveSix(patterns.map(_.sorted), withZeroThreeNine) ++ withZeroThreeNine
      allKnown
    }

    val numbers = input.map { case s"$tenNumbers | $fourNumbers" =>
      Numbers(tenNumbers.split(" ").toSeq, fourNumbers.split(" ").toSeq)
    }
