...
class Day5 {
  ...
  def solve(input: Seq[String]): (Long, Long) = {
    ...
    def pointsCoveredByLines(lines: Seq[Line]): Seq[Point] = lines.flatMap { line =>
    ...
    def countOverlap(points: Seq[Point]): Int = points
      .foldLeft[Map[Point, Int]](Map.empty) { case (acc, point) =>
    ...
    val inputLines = input.map { case s"$x1,$y1 -> $x2,$y2" =>
      Line(Point(x1.toInt, y1.toInt), Point(x2.toInt, y2.toInt))
    }

    val straightLines = inputLines.filterNot(_.isDiagonal)
