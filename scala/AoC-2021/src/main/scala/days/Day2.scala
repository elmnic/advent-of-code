...
class Day2 {
  ...
  def solve(input: Seq[String]): (Long, Long) = {
    ...
    val resultPart1 = input.map { case extractCommand(direction, units) => direction -> units.toInt }
      .groupBy(_._1)
    ...
    val resultPart2 = input.map { case extractCommand(direction, units) => Command(direction, units.toInt) }
      .foldLeft(Ship(Point(0, 0), 0)) { (ship, command) =>
        command match {
